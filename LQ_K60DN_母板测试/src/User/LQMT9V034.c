...
修改历史：
20181211-增加神眼摄像头在K60单片机使用TFT1.8屏显示二值化图像，可以选择OLED和TFT1.8
-------------------------------------------------------------
LED接口定义：
LED1--PTA17
LED2--PTC0
...
u8 Pixle[LCDH][LCDW];              //二值化后用于OLED显示的数据  //u16
...
{ 
    ...
    { 
        ...
        {    
              ...
              Pixle_Filter();
              Draw_Road();                         //龙邱OLED模块显示动态图像 ， TFT和OLED二选一
              //SendPicture();//发送到上位机    //使用上位机时打开此函数，使用OLED显示时关闭此函数，打开上面OLED显示
            //EnableInterrupts;        
        }    
    }
}
