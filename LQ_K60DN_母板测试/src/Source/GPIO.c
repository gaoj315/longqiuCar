...
/*
void PORTC_Interrupt(void)     //行场中断的触发
{
  short int i,j;
  int n;
  n=6;    //行中断
  if((PORTC_ISFR & (1<<n)))
  {
      PORTC_ISFR |= (1<<n); 
      // 用户自行添加中断内程序 //
      //if((++ROW_Count)%2 == 0)                                  //判断该行数据是否需要,根据自己需要的行数自己设定判别条件
      {
        ROW_Count++;
  ...
  if((PORTC_ISFR & (1<<n)))
  {
      ...
      if(FieldCounter == 1)
      {
        DMA_PORTx2BUFF_Init(DMA_CH4, (void *)&PTD_BYTE0_IN, Image_data, PTA19, DMA_BYTE1, LIN, DMA_rising_keepon); 
      ...
      else
      {
         DMA_DIS(DMA_CH4); 
         ROW_Count=0;  
//         UART_Put_Char(UART0,0xff);//发送帧头标志  发送图像到PC上位机
//         for(i=0;i<ROW;i++) 
//         {
//           for(j=0;j<LIN;j++)
//            {
...
//                  Image_data[i][j]=0xfe;
//               }
//              UART_Put_Char(UART0,Image_data[i][j]);
//            }
//         }
